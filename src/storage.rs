...
use crate::harness::{ModelPreferences, SessionBackend};
...
impl ModelPreferences for WorkspacePreferences {
    fn effort(&self, provider: &str, model: &str) -> Option<&str> {
        WorkspacePreferences::effort(self, provider, model)
    }

    fn reasoning_efforts(&self) -> BTreeMap<String, BTreeMap<String, String>> {
        WorkspacePreferences::reasoning_efforts(self)
    }
    ...
    fn set_effort(
        ...
        WorkspacePreferences::set_effort(self, provider, model, effort, default_effort)
    }
}
...
impl SessionBackend for SessionManager {
    fn current_id(&self) -> &str {
        SessionManager::current_id(self)
    }
    ...
    fn append_payloads(
        ...
        SessionManager::append_payloads(self, payloads, usage)
    }
    ...
    fn load(&mut self, requested_id: &str) -> Result<(Vec<SessionEntry>, Usage)> {
        SessionManager::load(self, requested_id)
    }
}
